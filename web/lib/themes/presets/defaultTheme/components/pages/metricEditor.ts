...
export const metricEditor = {
  ...
  // Inline autocomplete ghost text — Copilot-style. Rendered by a
  // *second* overlay on top of the syntax-highlight overlay: the prefix
  // up to the caret is painted with `color: transparent` (so the layout
  // takes the same horizontal space as the real text and the ghost
  // appears at the caret's position), and the suggestion suffix uses
  // this style. Same monospace font as the textarea so the ghost lines
  // up with the user's typing pixel-for-pixel. The mid-grey reads as
  // "preview, not committed" against the white wrapper surface.
  formulaGhost: {
    color: BEVEL_SHADOW,
  } as CSSProperties,
} as const;
