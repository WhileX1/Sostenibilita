...
Lives in its own file because the algorithm is independent of evaluation: it can run on a parsed AST without input values, and the editor uses it both at compile-time (to render the slider rows) and to drive the rating computation.
